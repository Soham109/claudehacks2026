...
export async function findMatchesForUser(
  ...
  // Try to join an existing forming table first
  ...
  // Find candidates — relaxed query: get ALL other onboarded users, then score them
  ...
  const candidates = allUsers.map((u) => {
    ...
    const freeBlocks = u.scheduleBlocks.filter((b) => b.day === day && b.isFree);
    if (freeBlocks.some((b) => b.startTime <= preferredTime && b.endTime >= preferredTime)) {
      score += 20;
    }
    ...
    return {
      userId: u.id, name: u.name, year: u.year, major: u.major,
      interests: theirInterests, bio: u.bio, lat: u.lat, lng: u.lng,
  ...
  const memberLocations = [
    ...
    ...topMatches.filter((m) => m.lat && m.lng).map((m) => ({ lat: m.lat!, lng: m.lng! })),
  ];
  ...
  // Create table — even if 0 matches, still create with just the user
  ...
  const table = await prisma.diningTable.create({
    data: {
      ...
      time: preferredTime,
      maxSize: 4,
      status: topMatches.length >= 2 ? "active" : "forming",
      icebreaker,
      tableSpot,
      aiPrompt,
